...
// 支店ごとに異なる電話番号はJSにハードコードせず、
// 各ページのform-note内 <span id="contactTelNumber"> から取得する。
// 該当spanが無いページでもエラーにならず、電話番号なしのメッセージにフォールバックする。
function getContactTelMessage() {
  const el = document.getElementById('contactTelNumber');
  const tel = el ? el.textContent.trim() : '';
  return tel ? `\nTEL: ${tel}` : '';
}
...
document.getElementById('recruitForm').addEventListener('submit', async function (e) {
  ...
  const submitIcon = '<svg viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2"><line x1="22" y1="2" x2="11" y2="13"/><polygon points="22 2 15 22 11 13 2 9 22 2"/></svg> 応募する';
  ...
  try {
    ...
    if (json.success) {
      ...
      alert(`送信に失敗しました。お手数ですが、お電話にてご連絡ください。${getContactTelMessage()}`);
      btn.disabled = false;
      btn.innerHTML = submitIcon;
    }
  } catch (err) {
    alert(`通信エラーが発生しました。お手数ですが、お電話にてご連絡ください。${getContactTelMessage()}`);
    btn.disabled = false;
    btn.innerHTML = submitIcon;
  }
});